...
<ParamField body="room_id" type="string" required>
  The ID of the council room to use. Get available rooms from `GET /api/v1/x402/rooms`.
</ParamField>
...
<Warning>
  **Keep Your Task ID Private!**

  The task ID returned is your **private access token** to view this debate. Anyone with this ID can access the task status, prompt, and results.

  - Do NOT share task IDs publicly (logs, URLs, screenshots)
  - Store task IDs securely (like API keys or passwords)
  - Task IDs are UUIDs (impossible to guess) - treat them as secrets

  This is intentional: x402 is stateless, so the task ID IS your proof of ownership.
</Warning>
...
```json
{
  ...
  "accepts": [{
    ...
    "resource": "http://thinkexchange.ai/x402/tasks",
    "description": "Create an AI Council debate task with multiple AI agents",
...
1. **Check Status:** Poll `GET /api/v1/x402/tasks/:id` to check completion status (FREE)
2. **Get Messages:** Retrieve full debate conversation with `GET /api/v1/x402/tasks/:id/messages` (FREE)
3. **WebSocket:** Connect to WebSocket URL for real-time updates
4. **Wait:** Typical debates complete in 30-120 seconds
...
- `GET /api/v1/x402/rooms` - List all available council rooms (FREE)
- `GET /api/v1/x402/tasks/:id` - Get task status and details (FREE)
- `GET /api/v1/x402/tasks/:id/messages` - Get debate conversation (FREE)

All endpoints except task creation are free to call.
